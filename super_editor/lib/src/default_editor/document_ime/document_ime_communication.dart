...
class _DocumentToImeSynchronizerState extends State<DocumentToImeSynchronizer> {
  ...
  void _onDocumentChange(DocumentChangeLog change) {
    editorImeLog.finer("[DocumentToImeSynchronizer] - document change. Sending document and selection to IME on the next frame.");
    _sendDocumentToImeOnNextFrame();
  }

  void _onSelectionChange() {
    if (widget.selection.value != null) {
      editorImeLog.finer("[DocumentToImeSynchronizer] - selection change. Sending document and selection to IME on the next frame.");
      _sendDocumentToImeOnNextFrame();
    }
  }
  ...
  void _onImeConnectionChange() {
    if (widget.imeConnection.value != null && widget.imeConnection.value!.attached) {
      ...
      editorImeLog.fine("[DocumentToImeSynchronizer] - An IME connection was just opened. Sending current document and selection to IME.");
      _sendDocumentToImeOnNextFrame();
    }
  }
  ...
  void _sendDocumentToIme() {
    ...
    editorImeLog.fine("[DocumentToImeSynchronizer] - Adding invisible characters?: ${imeSerialization.didPrependPlaceholder}");
...
class DocumentImeInputClient extends TextInputConnectionDecorator with TextInputClient, DeltaTextInputClient {
  ...
  void updateEditingValueWithDeltas(List<TextEditingDelta> textEditingDeltas) {
    ...
    _hasOutstandingMutatingChanges = textEditingDeltas.where((element) => element is! TextEditingDeltaNonTextUpdate).toList().isNotEmpty;
